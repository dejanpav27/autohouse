...
function DealerStorefront({ dealer, cars, onBack, isPublic }) {
  ...
  const sendToAI = async (userMessage, isWelcome = false) => {
    ...
    if (webhookUrl) {
      ...
      // ── Price-based search (handles: "25000", "25000e", "25.000€", "do 20000", "under 30k", "za 15 hiljada") ──
      const priceMatch = q.match(/(\d[\d.,]*)\s*(eur|€|euro|e|hiljad|k\b)?/i) || q.match(/(za|under|do|bis|pod|budget|max)\s*(\d[\d.,]*)/i);
      if (priceMatch && !reply) {
        let rawNum = (priceMatch[2]?.match(/\d/) ? priceMatch[2] : priceMatch[1]) || priceMatch[1];
        let budget = parseFloat(rawNum.replace(/\./g,"").replace(",",".")) || 0;
        if (budget > 0) {
          if (q.includes("k") || q.includes("hiljad")) budget *= 1000;
          if (budget < 500) budget *= 1000; // user probably meant thousands (e.g. "25" = 25000)
          matchedCars = available.filter(c => c.price <= budget).sort((a,b) => b.price - a.price).slice(0, 5);
          if (matchedCars.length === 0) {
            const cheapest = available.sort((a,b) => a.price - b.price).slice(0, 3);
            reply = `No vehicles found under €${budget.toLocaleString()}. Our most affordable options start from €${cheapest[0]?.price?.toLocaleString()}:\n\n${cheapest.map((c,i) => `${i+1}. **${c.make} ${c.model}** — €${c.price?.toLocaleString()}`).join("\n")}`;
            if (cheapest.length > 0) { setSelectedCar(cheapest[0]); if (cheapest[1]) setCompareCar(cheapest[1]); }
          }
        }
      }

      // ── Make AND model search (handles: "golf", "bmw", "audi a4", "volkswagen golf") ──
      if (!matchedCars.length && !reply) {
        const words = q.split(/\s+/);
        // Search in both make and model
        const found = available.filter(c => {
          const carText = `${c.make} ${c.model} ${c.trim || ""}`.toLowerCase();
          return words.some(w => w.length >= 2 && carText.includes(w));
        });
        if (found.length > 0) matchedCars = found;
      }
      
      // ── Fuel type search ──
      if (!matchedCars.length && !reply) {
        if (/diesel|dizel/i.test(q)) matchedCars = available.filter(c => /diesel/i.test(c.fuel));
        else if (/benzin|gasoline|petrol|benzinac/i.test(q)) matchedCars = available.filter(c => /gasoline|petrol|benzin/i.test(c.fuel));
        else if (/hybrid|hibrid/i.test(q)) matchedCars = available.filter(c => /hybrid/i.test(c.fuel));
        else if (/elektr|electric|struja/i.test(q)) matchedCars = available.filter(c => /electric|elektr/i.test(c.fuel));
      }

      // ── Body type ──
      if (!matchedCars.length && !reply) {
        if (/suv|džip|jeep|dzip/i.test(q)) matchedCars = available.filter(c => /suv|jeep|crossover/i.test(c.bodyType));
        else if (/sedan|limuzin/i.test(q)) matchedCars = available.filter(c => /sedan/i.test(c.bodyType));
        else if (/karavan|wagon|kombi|estate/i.test(q)) matchedCars = available.filter(c => /wagon|kombi|estate|karavan/i.test(c.bodyType));
        else if (/hatch|hečbek|hecbek/i.test(q)) matchedCars = available.filter(c => /hatch/i.test(c.bodyType));
        else if (/kupe|coupe|cabrio|kabriolet/i.test(q)) matchedCars = available.filter(c => /coupe|cabrio|kupe/i.test(c.bodyType));
      }

      // ── Family / space ──
      if (!matchedCars.length && !reply) {
        if (/famil|porodic|family|kinder|deca|djeca|prostran|space|veliko/i.test(q)) {
          matchedCars = available.filter(c => /suv|wagon|van|kombi|crossover/i.test(c.bodyType) || c.drivetrain === "AWD").slice(0, 5);
        }
      }

      // ── Automatic / manual ──
      if (!matchedCars.length && !reply) {
        if (/automat|automatic/i.test(q)) matchedCars = available.filter(c => /auto/i.test(c.transmission));
        else if (/manual|manuelni|ručni|rucni/i.test(q)) matchedCars = available.filter(c => /manual/i.test(c.transmission));
      }

      // ── Color ──
      if (!matchedCars.length && !reply) {
        const colors = ["white","black","red","blue","gray","grey","silver","green","brown","beige","bela","crna","crvena","plava","siva","srebrna","zelena"];
        const colorMap = {bela:"white",crna:"black",crvena:"red",plava:"blue",siva:"gray",srebrna:"silver",zelena:"green"};
        const foundColor = colors.find(c => q.includes(c));
        if (foundColor) {
          const en = colorMap[foundColor] || foundColor;
          matchedCars = available.filter(c => c.color?.toLowerCase().includes(en) || c.color?.toLowerCase().includes(foundColor));
        }
      }

      // ── Test drive / booking ──
      if (!reply && /test|drive|prob|vožn|vozn|termin|zakaž|zakaz|book|besichtig/i.test(q)) {
        if (selectedCar) {
          ...
          reply = "Which vehicle would you like to test drive? Tell me a make or model and I'll help you book.";
        }
      }

      // ── Specs / details ──
      if (!reply && /spec|detail|info|podatk|characteristic|übersicht|detalj|osobin/i.test(q)) {
        if (selectedCar) {
          setShowSpecs(true);
          reply = `Here are the full specs for the **${selectedCar.make} ${selectedCar.model}**. I've opened the overview panel for you.`;
        } else {
          reply = "Tell me which vehicle you're interested in and I'll show you all the details!";
        }
      }

      // ── Compare ──
      if (!reply && /compar|poredi|vergleich|razlik|differ|vs\b/i.test(q) && selectedCar && compareCar) {
        reply = `Comparing **${selectedCar.make} ${selectedCar.model}** (€${selectedCar.price?.toLocaleString()}, ${selectedCar.hp}hp) with **${compareCar.make} ${compareCar.model}** (€${compareCar.price?.toLocaleString()}, ${compareCar.hp}hp). The ${selectedCar.price > compareCar.price ? compareCar.make : selectedCar.make} offers better value, while the ${selectedCar.hp > compareCar.hp ? selectedCar.make : compareCar.make} has more power.`;
      }

      // ── Greeting ──
      if (!reply && !matchedCars.length && /^(hi|hello|hey|zdravo|cao|ćao|hej|pozdrav|dobar dan|guten tag|hallo)\b/i.test(q)) {
        const top = available.slice(0, 2);
        reply = `Welcome to **${dealer.name}**! We have **${available.length} vehicles** in stock. What are you looking for? You can ask by:\n\n• **Budget** — "25000" or "do 15000€"\n• **Make/Model** — "Golf", "BMW", "Audi A4"\n• **Type** — "SUV", "diesel", "automatic"`;
        if (top.length > 0) { setSelectedCar(top[0]); if (top[1]) setCompareCar(top[1]); }
      }

      // ── Build reply from matched cars ──
      if (!reply && matchedCars.length > 0) {
        const top = matchedCars.slice(0, 5);
        if (top.length === 1) {
          reply = `Found a match: **${top[0].make} ${top[0].model}** (${top[0].year || "N/A"}) for **€${top[0].price?.toLocaleString()}** — ${top[0].mileage?.toLocaleString()}km, ${top[0].fuel}, ${top[0].hp}hp. Would you like to see the full specs or book a test drive?`;
        } else {
          reply = `Found ${matchedCars.length} vehicle${matchedCars.length > 1 ? "s" : ""}:\n\n${top.map((c,i) => `${i+1}. **${c.make} ${c.model}** (${c.year || "N/A"}) — €${c.price?.toLocaleString()}, ${c.mileage?.toLocaleString()}km, ${c.fuel}`).join("\n")}${matchedCars.length > 5 ? `\n\n...and ${matchedCars.length - 5} more. Narrow it down by adding more details.` : ""}`;
        }
      ...
      // ── Fallback ──
      if (!reply) {
        if (isWelcome || chatMessages.length <= 1) {
          const top = available.slice(0, 2);
          reply = `Welcome to **${dealer.name}**! We have **${available.length} vehicles** available.\n\nYou can ask me:\n• A budget — just type a number like **25000**\n• A make or model — **Golf**, **BMW**, **Audi A4**\n• A type — **SUV**, **diesel**, **automatic**, **family car**\n• **Book a test drive** for any vehicle`;
          if (top.length > 0) { setSelectedCar(top[0]); if (top[1]) setCompareCar(top[1]); }
        } else {
          reply = `I couldn't find a match for "${userMessage}". Try:\n• A number for budget (e.g. **20000**)\n• A car name (e.g. **Golf**, **BMW**)\n• A category (e.g. **SUV**, **diesel**)`;
        }
      }
